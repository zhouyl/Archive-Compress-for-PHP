...
class Archive_Tar extends Archive
{
    ...
    public function __construct($options = null)
    {
        if (! class_exists('Archive_Tar')) {
            throw new Archive_Exception('This filter needs PEARs Archive_Tar');
        }
    ...
    public function setMode($mode)
    {
        ...
        if (($mode != 'Bz2') && ($mode != 'Gz')) {
            throw new Archive_Exception("The mode '{$mode}' is unknown");
        }

        if (($mode == 'Bz2') && (! extension_loaded('bz2'))) {
            throw new Archive_Exception('This mode needs the bz2 extension');
        }

        if (($mode == 'Gz') && (! extension_loaded('zlib'))) {
            throw new Archive_Exception('This mode needs the zlib extension');
        }
    }
    ...
    public function compress($content)
    {
        ...
        if (! file_exists($content)) {
            ...
            if ($result === false) {
                throw new Archive_Exception('Error creating the temporary file');
            }
        ...
        if ($result === false) {
            throw new Archive_Exception('Error creating the Tar archive');
        }
    ...
    public function decompress($content)
    {
        ...
        if (file_exists($content)) {
            ...
            throw new Archive_Exception('Tar Archive not found');
        }
        ...
        if ($result === false) {
            throw new Archive_Exception('Error while extracting the Tar archive');
        }
