...
class Archive_Zip extends Archive
{
    ...
    public function __construct($options = NULL)
    {
        if (! extension_loaded('zip')) {
            throw new Archive_Exception('This filter needs the zip extension');
        }
    ...
    public function compress($content)
    {
        ...
        if ($res !== TRUE) {
            throw new Archive_Exception($this->_errorString($res));
        }
    ...
    public function decompress($content)
    {
        ...
        if (file_exists($content)) {
            ...
            throw new Archive_Exception('ZIP Archive not found');
        }
        ...
        if (empty($target) || ! is_dir($target)) {
            throw new Archive_Exception('No target for ZIP decompression set');
        }
        ...
        if (version_compare(PHP_VERSION, '5.2.8', '<')) {
            for ($i = 0; $i < $zip->numFiles; $i++) {
                ...
                if (($currName{0} == '/') OR
                    (substr($currName, 0, 2) == '..') OR
                    (substr($currName, 0, 4) == './..')
                    )
                {
                    throw new Archive_Exception("Upward directory traversal was detected inside {$archive}"
